...
client.on('messageCreate', async (message) => {
 ...
 if (match) {
  const gameNumber = parseInt(match); // Get the game number from the first capturing group
  const score = parseInt(match) || 'X'; // Get the score from the second capturing group
  const grid = match; // Get the grid from the third capturing group
